const GOOGLE_SHEETS_WEB_APP_URL = 'https://script.google.com/macros/s/AKfycbym4rVEUWd0xkp9JglZNjZp6Hse6IxGSkHgqqKsi05GJhwe2AD95Z1-bGCv7dhWMLBqXQ/exec'; // تأكد أن هذا الرابط هو الصحيح لتطبيق الويب الخاص بك
...
// **تعديل: وظيفة التحقق من صحة حقول منتجات الجرد وفقاً للشرط الجديد**
function validateInventoryItems() {
  ...
  let allValid = true;
  let validationMessages = []; // لتجميع رسائل التحقق
  ...
  items.forEach((itemDiv, index) => {
    ...
    const unitLabel = unitLabelSelect.value.trim();

    // إزالة أي تمييز سابق
    productNameInput.classList.remove('border-red-500');
    quantityInput.classList.remove('border-red-500');
    unitLabelSelect.classList.remove('border-red-500');

    // الشرط الجديد: إذا تم اختيار منتج، فاجعل الكمية والوحدة إلزامية
    if (productName !== '') {
      if (quantity === '') {
        allValid = false;
        quantityInput.classList.add('border-red-500');
        validationMessages.push(`يرجى إدخال الكمية للمنتج "${productName}" (العنصر ${index + 1}).`);
      } else if (parseInt(quantity) < 1) { // التحقق من أن الكمية لا تقل عن 1 إذا كانت مدخلة
        allValid = false;
        quantityInput.classList.add('border-red-500');
        validationMessages.push(`كمية المنتج "${productName}" (العنصر ${index + 1}) يجب أن تكون 1 أو أكثر.`);
      }

      if (unitLabel === '') {
        allValid = false;
        unitLabelSelect.classList.add('border-red-500');
        validationMessages.push(`يرجى اختيار الوحدة للمنتج "${productName}" (العنصر ${index + 1}).`);
      }
    }
    // إذا لم يتم اختيار منتج، فالحقول الأخرى اختيارية (لا تحتاج لتحقق)
  });

  if (!allValid) {
    // عرض جميع رسائل التحقق المجمعة
    showWarningMessage('الرجاء تصحيح الأخطاء التالية:\n' + validationMessages.join('\n'));
    // التمرير إلى أول حقل غير صالح إذا أمكن
    const firstInvalid = inventoryItemsContainer.querySelector('.border-red-500');
    if (firstInvalid) {
...
async function handleSubmit(event) {
  ...
  if (selectedVisitType === 'جرد استثنائي') {
    // التحقق من صحة حقول الجرد قبل الإرسال بالمنطق الجديد
    ...
    inventoryItemsContainer.querySelectorAll('.inventory-item').forEach(itemDiv => {
      ...
      // فقط إذا كان اسم المنتج موجودًا، قم بجمع بياناته.
      // هذا يسمح بإرسال حقول جرد فارغة تمامًا إذا لم يتم اختيار منتج.
      if (productName.trim() !== '') {
          let productDetails = {};
          if (selectedOption) {
    ...
    // بناء الـ payload لبيانات الجرد (قد تكون مصفوفة فارغة إذا لم يتم إدخال أي منتجات)
    payload = {
  ...
  try {
    ...
    // إضافة عنصر الجرد الأولي فقط إذا كان نوع الزيارة الحالي "جرد استثنائي"
    // أو لإعادة الحالة الافتراضية إذا كان هناك تغيير نوع زيارة.
    // بما أننا سمحنا بالجرد الفارغ، لن نضيف العنصر الأولي بشكل إلزامي هنا بعد إعادة التعيين.
    // يمكن للمستخدم إضافة عنصر يدوياً بالضغط على زر "إضافة منتج جرد".


    // إعادة ضبط الأقسام المرئية بناءً على نوع الزيارة الافتراضي بعد إعادة التعيين
    // هذا سيضمن عرض القسم الصحيح (سواء زيارة عادية أو جرد استثنائي)
...
function toggleVisitSections(selectedType) {
  if (selectedType === 'جرد استثنائي') {
    ...
    // مسح عناصر الجرد
    inventoryItemsContainer.innerHTML = '';
    // **لم نعد نضيف العنصر الأولي هنا بشكل إلزامي بعد التبديل**
    // يمكن للمستخدم إضافة عناصر الجرد يدوياً إذا رغب.
  }
}
...
// إضافة قالب جديد لعنصر الجرد (بدون required أو min="1")
  ...
  const template = `
    <div class="inventory-item border border-yellow-200 p-4 rounded-lg bg-white relative">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="form-group">
          <label>الكمية</label>
          <input type="number" name="Inventory_Quantity" placeholder="أدخل الكمية" />
        </div>
        <div class="form-group">
...
// **تم تعديل هذا ليسمح بوجود صفر عناصر إذا رغب المستخدم بحذفها كلها**
function addInitialInventoryItem() {
  // نضيف العنصر الأولي فقط إذا كانت الحاوية فارغة تماماً
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // **تم إزالة addInitialInventoryItem() من هنا لكي لا يكون هناك عنصر جرد إلزامي عند التحميل**
  // يمكن للمستخدم إضافة العنصر الأول يدوياً أو تركه فارغاً.
  // سنقوم بإضافة عنصر واحد فقط بشكل مبدئي إذا كانت القائمة فارغة تماما (لتوفير واجهة مستخدم بديهية).
  ...
  inventoryItemsContainer.addEventListener('click', (event) => {
    if (event.target.classList.contains('removeInventoryItem')) {
      // **تعديل: السماح بالحذف حتى لو كان العنصر الوحيد متبقياً**
      // إذا كان هناك عناصر جرد يمكن حذفها
      if (inventoryItemsContainer.children.length > 0) {
        event.target.closest('.inventory-item').remove();
      }
      // لا حاجة لرسالة تحذير لأننا سمحنا بحذف جميع العناصر
    }
  });
